...
\footnotetext{En klassisk vätska är en vätska som uppfyller Stokes lag för viskös friktion -- alltså att friktionskraften är linjärt beroende av hastigheten genom fluiden.}
...
Jämförs exponenterna i potensanpassningen mellan cellerna i dvala och log-fas, (a) respektive (b) i \figref{fig:MSD}, finns en viss skillnad. Exponenten för cellerna i dvala är $0,60$ mot log-fas-cellernas $0,80$. Med osäkerhetsgränser som i \figref{fig:MSD_error} syns att dessa värden på exponenterna själva får en osäkerhet på ungefär $\pm 0,05$. Även om \figref{fig:MSD_error} bara visar detta för partiklar från celler i dvala så gäller samma osäkerhet i exponenten för celler i log-fas.
...
\subsubsection{Likhet mellan de olika måtten på MSD antyder att fBm är en bättre modell}
...
Då de undersökta partikelrörelserna visar upp överensstämmande resultat mellan $S$ och $s$, se \figref{fig:MSD}, tyder detta på att partiklarna utför en rörelse med stationära steg. Baserat på denna observation utgör fBm en bättre modell för partikelrörelsen än CTRW.
...
En annan anledning till att brusig data inte kan förklara avvikelsen för celler i dvala i \figref{fig:asymmetri}, är att partiklarna från celler i log-fasen inte påverkades trots att brusnivån borde vara samma i båda fallen. Man skulle kunna argumentera för att $\nicefrac{\sigma_\text{brus}}{\sigma_\text{steg}}$ ändå kan skilja sig mellan de olika cellfaserna för att $\sigma_\text{steg}$ skulle kunna variera. \figref{fig:storleksberoende}~(c) och (d) visar dock tydligt hur steglängderna mer eller mindre är samma mellan cellfaserna.
