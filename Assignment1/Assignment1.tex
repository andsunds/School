...
=\alpha^3\ee^{-\alpha r} 
\end{equation}
The charge distribution is therefore
\begin{equation}
\rho(r)=-\enaught\laplacian\Phi=
q\frac{\alpha^3}{4\pi}\ee^{-\alpha r}
\end{equation}
This would correspond to $q$ times the electron's probability density
around the nucleus for a ``1s'' state. It is also quite easy to see that 
\begin{equation}
\iiint \rho(r)\id{V}=q,
\end{equation}
which means that we have one ``whole'' electron here --- as we
would expect. 
