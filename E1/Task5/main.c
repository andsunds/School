...
int main()
{
	...
	double (*E)[2] = malloc( (nbr_of_timesteps+1) * 2 * sizeof (double));
	double *p_spectrum = malloc((nbr_of_timesteps+1) * sizeof (double));
	...
	for (i = 1; i < nbr_of_timesteps + 1; i++) {
		...
		E[i][1] = calc_ke(v, nbr_of_particles, m);
	...
	powerspectrum_shift(p_spectrum, nbr_of_timesteps+1);
