...
However \figref{fig:good-fit} do seems to suggest that a dual
...
parameters. 
\footnotetext{See 
  \url{https://se.mathworks.com/help/curvefit/fit.html?requestedDomain=www.mathworks.com}. 
This is also where the 95\,\% confidence intervals in the other
presented results come from.}
...
\caption{Represeantative waveform of pulse echo peaks, with the
  ...
  $\tau$. The noise background level, has not been canceld in this
  plot. } 
\label{fig:waveform}
\end{figure*}
...
\caption{Detected pulse echo peaks together with singel and dual mode
  decay fits. Here the noise background level have been subtracted
  from the heights of the peaks. If the decay had been single moded,
  then we would have expected that the peaks fall on a line here in
  the log-plot, as shown by the single mode decay fit.  }
\label{fig:peaks-exwpp}
\end{figure*}
