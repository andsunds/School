...
\title{A qualitative investigation of self-diffusion in liquid water
  and glycerol and wet paper using {$^1$H}~NMR pulse echoes} 
...
\maketitle
\begin{abstract}
In this report the {$^1$H}~NMR pulse echo method is used to study
...
\footnotetext{A \textsl{TeachSpin PS1-A}. }
...
modes are shown in \figref{fig:fitted-peaks} in
...
\figref{fig:water-pos}. The first one is that the static magnetic 
...
\figref{fig:water-pos}, but instead of coming from two different
sample positions the two lines would have arisen from a fast and slow
mode of diffusion. This is not what we see. \figref{fig:dual-diff}
...
presented in \figref{fig:good-fit} are however the so called
...
at all.
\footnotetext{The values of $1-r^2$ for extra wet paper in
  \figref{fig:good-fit} is nearly an order of magnitude apart between
  the two fits. And the peaks in the example shown in
  \figref{fig:fitted-peaks} clearly do not fall on a straight line. }
...
\caption{Detected pulse echo peaks from extra wet paper and liquid
  ...
  the peaks. For a true single mode decay the peaks would all fall on
  a straight line in this log-plot, as shown by the liquid water
  peaks. This is clearly also \emph{not} the case for the extra wet
  paper. } 
...
%  LocalWords:  Matlab's Meiboom Matlab TeachSpin
