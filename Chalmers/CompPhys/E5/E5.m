...
clf;
plot(rr, U_test , '-',rr, U_Hartree, ':k', 'linewidth',4)
xlabel('r [a_0]');
ylabel('U');
...
[emin,i] = min(min(D));
E0 = D(i,i)
plot(rr, abs(F(:,i))./(sqrt(4*pi).*rr)/sqrt(dr),'r', rr, sqrt(n_s), ':k','linewidth',2)
xlabel('r [a_0]');
ylabel('f(r)');
...
legend('Z = 2, Hartree', 'Z = 2, central-field approx', 'Z = 27/16, central-field approx')
