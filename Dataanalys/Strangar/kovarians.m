...
H = zeros(N,k);
for j=1:k
    H(:,j) = [ones(h(j),1);NaN(N-h(j),1)]; % Placera ettor de som ska va kvar och NaN p� de som ej ska va med.
end
T=fliplr(K(:,index)).*H; % Flippa K f�r att placera f�rsta moden i kolumn 1, andra i kolumn 2 etc.
